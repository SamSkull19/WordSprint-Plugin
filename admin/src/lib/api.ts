...
export function listWords(params: { search?: string; letter_pattern?: string; page?: number; per_page?: number }): Promise<WordsListResponse> {
	const query = new URLSearchParams();
	if (params.search) query.set('search', params.search);
	if (params.letter_pattern) query.set('letter_pattern', params.letter_pattern);
